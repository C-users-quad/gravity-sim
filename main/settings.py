import pygame
import sys
import math
import pygame_gui
from random import randint

WINDOW_WIDTH, WINDOW_HEIGHT = 1040, 585
HALF_WORLD_WIDTH, HALF_WORLD_HEIGHT = 5000, 5000
BORDER_WIDTH = 30
NUM_PARTICLES = 1000
RENDER_DISTANCE = 1500
CELL_SIZE = 150

G = 10

MIN_RADIUS, MAX_RADIUS = 2, 249
MAX_STARTING_VELOCITY = 100
MAX_STARTING_MASS = 1000000
MAX_STARTING_DENSITY = 10 # mass per radius
PARTICLE_SPEED_AFTER_DRAGGING = 50
PARTICLE_SPEED_AFTER_DRAGGING_UNCHANGED = True # determines if "PARTICLE_SPEED_AFTER_DRAGGING" is actually used

FPS = 120

INFO_RECT_PADDING = 5
INFO_RECT_COLOR = (16, 17, 18, 200)

MIN_CAM_SPEED, MAX_CAM_SPEED = 150, 2500
MIN_ZOOM, MAX_ZOOM = 0.1, 2
SCROLL_WHEEL_SENSITIVITY = 50

BG_COLOR = '#17171a'
BUTTON_COLOR = "#4D64D6"
HIGHLIGHT_COLOR = "#6BFD11"
BORDER_COLOR = (240, 240, 240)
INPUT_BOX_LENGTH = 50